...
inline const char *shortFile(const char *file)
{
    ...
    const char *ptr = std::strstr(file, "chad-engine/");
    if (ptr != nullptr) {
        ptr += 12;
    } else {
        ptr = file;
...
#define LOG_INFO(fmt, ...) \
    do { \
        fprintf( \
            stdout, COLOR_INFO "[INFO] %s:%d: " fmt COLOR_RESET "\n", ::chad::shortFile(__FILE__), __LINE__, ##__VA_ARGS__); \
...
#define LOG_WARN(fmt, ...) \
    do { \
        fprintf( \
            stderr, COLOR_WARN "[WARN] %s:%d: " fmt COLOR_RESET "\n", ::chad::shortFile(__FILE__), __LINE__, ##__VA_ARGS__); \
...
#define LOG_ERROR(fmt, ...) \
    do { \
        fprintf( \
            stderr, COLOR_ERROR "[ERROR] %s:%d: " fmt COLOR_RESET "\n", ::chad::shortFile(__FILE__), __LINE__, ##__VA_ARGS__); \
...
#define LOG_FATAL(fmt, ...) \
    do { \
        fprintf( \
            stderr, COLOR_FATAL "[FATAL] %s:%d: " fmt COLOR_RESET "\n", ::chad::shortFile(__FILE__), __LINE__, ##__VA_ARGS__); \
        abort(); \
    } while (0)
